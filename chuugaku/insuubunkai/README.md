...
このディレクトリは、中学生向けの二次式の因数分解練習問題を自動生成し、問題・模範解答を個別に印刷できるシングルページ（`insuubunkai.html`）を提供します。通常の `x^2 + bx + c` 型に加え、`(ax + p)(ax + q)` 型の問題も生成できます。

## 特長

- 問題の自動生成：指定範囲・条件に基づきランダム生成
- 通常問題と係数付き問題の切替：チェックボックスで出題形式を切替
- 重複防止：同じ問題を1回の生成結果内で重複させない
- 解答の体裁最適化：
  - 片方の因数が 0 のときは `x(x + a)` または `ax(ax + p)` 形式に整形
  - 重解のときは `(x + a)^2` または `(ax + p)^2`
- 印刷最適化：A4・2列レイアウト、問題のみ／解答のみ印刷
- 数式表示：問題・模範解答を MathJax（CDN）で見やすく表示
...
- 因数の範囲：定数項 `p, q` の範囲を `-20〜20` で指定（初期値: `-9〜9`）
- 負の数を含める：オンで `p, q` に負符号を含めます（オフで 0 以上に制限）
- 重解を含める：オンで `p = q` を許可。オフで `p ≠ q` のみ生成
- x に係数がある問題を作る：オンで `(ax + p)(ax + q)` 形のみ生成（`a` は `2〜9`）
- 問題数：`10問` / `20問` をトグルで選択
- ボタン：`問題を生成` / `プレビューを更新` / `問題だけ印刷` / `模範解答だけ印刷`

エラー時は入力欄下のメッセージに理由が表示されます（範囲の整合性や、重複なしで必要数を作れない場合など）。

## 出力と表示の仕様

- 問題：通常問題は `x^2 + bx + c = ________`、係数付き問題は `a^2x^2 + a(p+q)x + pq = ________` のアンダーライン形式
- 模範解答：右辺を以下の規則で表示します。
  - 重解：`(x + a)^2`（`a = 0` のときは `x^2`）
  - 片方が 0：`x(x + a)` または `ax(ax + p)`
  - それ以外：`(x + p)(x + q)` または `(ax + p)(ax + q)`
- 数式表示：問題・模範解答ともに MathJax により `\( ... \)` の TeX 形式をレンダリングします。
...
- 生成アルゴリズム：条件に合う候補を全列挙し、重複を除去してからシャッフル抽選します。
- 通常問題：`(x + p)(x + q)` を展開し、`b = p + q`, `c = p * q` を算出します。
- 係数付き問題：`(ax + p)(ax + q)` を展開し、`a^2x^2 + a(p + q)x + pq` を作ります。
- 解答表示の整形：係数付きモードを含めて `texFactorization(problem)` で体裁を整えます。
